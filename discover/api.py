...
def division_form(request):
    ...
    if request.method == 'POST':
        ...
        return fragments.division_form(request, division.team)
    data = request.GET
    if "team_slug" in data:
        team = CoachHelper.Get.team(data, owner)
...
def division_delete(request):
    ...
    if request.method == 'POST':
        ...
        if division:
            team = division.team
            division.delete()
            return fragments.handle_division_delete(request, team)
...
def registration_cancel(request):
    ...
    if request.method == 'POST':
        ...
        if registration.status != Registration.Status.DRAFT:
            raise ValueError("Only draft registrations can be cancelled.")
        registration.delete()
        return fragments.workspace_content(request, owner)
    data = request.GET
    if "registration_slug" in data:
    ...
    if request.method == 'POST':
        ...
        for team in owner.teams.all():
            ...
            for division in team.divisions.all():
                # Check if the division is selected for submission
